...
def get_parser():
    ...
    parser.add_argument("--output_dir", type=str, default="/sensei-fs-3/users/okara/camera_as_rays/RayDiffusion/CO3D_CHAIR")
...
def process_poses(co3d_dir, category, output_dir, min_quality):
    ...
    for subset in ["train", "test"]:
        ...
        # print(f"Writing {len(category_data)} sequences to {subset}")
        # print(category_data)
        # input()
        output_file = osp.join(args.output_dir, f"{args.category}_{subset}.jgz")
        print(output_file)
        print(args.output_dir)
        # output_file = '/sensei-fs-3/users/okara/camera_as_rays/RayDiffusion/CO3D_CHAIR/CO3D_CHAIR_train.jgz'
        # input()
        with gzip.open(output_file, "w") as f:
