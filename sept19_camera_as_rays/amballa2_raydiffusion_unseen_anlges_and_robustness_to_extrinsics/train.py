...
class Trainer(object):
    def __init__(self, cfg):
        ...
        self.train_dataloader = torch.utils.data.DataLoader(
            ...
            shuffle=True,
            num_workers=1,#cfg.training.num_workers,
            pin_memory=True,
            drop_last=True,
        )
        print("Dataloader length:", len(self.train_dataloader))
    ...
    def train(self):
        ...
        while self.iteration < self.max_iterations:
            for batch in self.train_dataloader:
                # batch = first_batch
                frame_names = [fil[0].split('/')[-1].split('.')[0] for fil in batch["filename"]]
                print(frame_names)
                t0 = time.time()
    ...
    def visualize(self, images, cameras_gt, crop_parameters=None):
        ...
        for i, cameras_pred in enumerate(cameras_pred_batched):
            fig = create_plotly_cameras_visualization(cameras_gt[i], cameras_pred, i, images=images[i])
