...
def create_plotly_cameras_visualization(cameras_gt, cameras_pred, num, images):
    ...
    # # Add images as 2D planes in the 3D scene
    # images = images.detach().cpu().numpy()
    # for i in range(num_frames):
    #     # Convert image to a format suitable for Plotly
    #     img = images[i]
    #     img = np.array(img)  # Ensure it's a NumPy array if not already
    #     _, img_height, img_width = img.shape

    #     # Position of the image in 3D space based on the camera position
    #     camera_position = cameras_gt.T[i].detach().cpu().numpy()
    #     import plotly.graph_objects as go
    #     # Create a 2D plane for the image
    #     fig.add_trace(go.Image(
    #         z=img,
    #         x0=camera_position[0] - img_width * camera_scale / 2,
    #         y0=camera_position[1] - img_height * camera_scale / 2,
    #         xaxis='x',
    #         yaxis='y',
    #         name=f"Image {i}",
    #         opacity=0.8
