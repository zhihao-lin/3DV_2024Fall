...
import sys
sys.path.append("/home/exx/Documents/SSD/Samsung8TB_2/chaitu/RayDiffusion/")
...
def process_predictions(eval_path, pred_index):
    ...
    """
    errors = {
        c: {n: {"CC": [], "R": []} for n in range(8,9)}
        for c in TRAINING_CATEGORIES 
    }

    for category in tqdm(TRAINING_CATEGORIES):
        for num_images in range(8,9):
    ...
    print("N=       ", " ".join(f"{i: 5}" for i in range(8, 9)))
    ...
    print("R mean", np.mean(np.array(errors[category][num_images]["R"])))
    print("CC mean", np.mean(np.array(errors[category][num_images]["CC"])))
...
if __name__ == "__main__":
    ...
    if eval_path is None:
        if eval_type == "diffusion":
            ...
            # eval_path = "output/working_diffusion_many_samples/"
            # eval_path = "output/corrupted_R_T_noise=0.01/"
            eval_path = "output/corrupted_R_T_noise=0.1/"
