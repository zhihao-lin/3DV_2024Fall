...
class SpatialDino(nn.Module):
    ...
    def forward(self, x, autoresize=False):
        ...
        *B, c, h, w = x.shape
        ...
        # add random noise to the features to corrupt the information
        # noise = torch.randn_like(features) #* 0.1
        # features = features + noise

        # completely corrupt features of the first image
        # features[0:2] = torch.randn_like(features[0:2])

        # completely corrupt features of the the first few patches for all the images
        # features[:, :, 0:2, 0:2] = torch.randn_like(features[:, :, 0:2, 0:2])
