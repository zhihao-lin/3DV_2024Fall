...
# import additional libraries needed for redering function
from hmr2.datasets.utils import expand_bbox_to_aspect_ratio
from phalp.configs.base import CACHE_DIR
from phalp.utils import get_pylogger
import joblib
...
class HMR2_4dhuman(PHALP):
    ...
    def render_from_pkl(self, pkl_file, video_path):
        ''' 
        self defined function to load the output pkl file and render the video
        modified from the PHLAP's track() function (https://github.com/brjathu/PHALP/blob/master/phalp/trackers/PHALP.py#L171-L268)
        but with data directly load from pkl instead of using network prediction
        input:
            pkl_file: the pkl file path that contains the result (either smoothed or original)
            video_path: the path to save the rendered video
        '''
        # load the output pkl file
        output = joblib.load(pkl_file)
        
        
        # follow the track() function in PHALP repo
        from copy import deepcopy
        final_visuals_dic = deepcopy(output)
        list_of_frames = []
        for t_, data in enumerate(output.items()):
            image_frame = self.io_manager.read_frame(data[0])
            list_of_frames.append(data[0])
            final_visuals_dic[data[0]]['frame'] = image_frame
...
def main(cfg: DictConfig) -> Optional[float]:
    ...
    phalp_tracker.render_from_pkl("outputs/results/demo_wrestling_I_smooth.pkl",'result_video/demo_wrestling_I_smooth.mp4')
    # phalp_tracker.track()
